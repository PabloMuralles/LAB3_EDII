...
namespace lab3.Estructura
{
    public class BTree<V, K> where K : IComparable<K>
    {
        ...
        /// <param name="padre">The parent node.</param>
        /// <param name="diviir">The node to split.</param>
        /// <param name="index">The index to split.</param>
        private void SplitBTreeNode(BtreeNode<K, V> padre, BtreeNode<K, V> diviir, int index)
        {
            BtreeNode<K, V> newBTreeNode = new BtreeNode<K, V>(order);
            padre.elements.Insert(index, diviir.elements[order/2]);
            padre.childs.Insert(index + 1, newBTreeNode);
            newBTreeNode.elements.AddRange(diviir.elements.GetRange(order, order - 1 ));
            diviir.elements.RemoveRange(order - 1, order);
            if (!diviir.IsLeaf)
            {
                newBTreeNode.childs.AddRange(diviir.childs.GetRange(order, order));
                diviir.childs.RemoveRange(order, order);
            }
        }
